      ...
      <main className="relative mx-auto max-w-[760px] px-4 pb-28 sm:px-6 lg:max-w-7xl lg:max-w-[1440px] lg:pb-12">
        ...
        <section id="outils" className="pt-16 lg:pt-24">
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-12 lg:grid-rows-[170px_170px_150px]">
            {tools.map((tool, index) => { const featured = index === 0; return <Link key={tool.title} href={tool.href} className={`group relative overflow-hidden rounded-[18px] border border-[#dbe9f4] bg-white shadow-[0_14px_32px_-24px_rgba(28,77,130,.6)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_38px_-22px_rgba(28,77,130,.55)] ${featured ? "col-span-2 min-h-[250px] lg:col-span-5 lg:row-span-2" : "min-h-[170px] lg:col-span-4"}`}><div className={`absolute inset-0 ${featured ? "lg:w-[58%]" : ""}`}>{tool.media.type === "video" ? <video src={tool.media.src} poster={"poster" in tool.media ? tool.media.poster : undefined} autoPlay muted loop playsInline preload="metadata" aria-label={tool.title} className={`h-full w-full transition duration-500 ${featured ? "bg-[#dceeff] object-contain object-center" : "object-cover group-hover:scale-105"}`} /> : <Image src={tool.media.src} alt={tool.title} fill className="object-cover transition duration-500 group-hover:scale-105" />}</div><div className={`absolute inset-0 ${featured ? "bg-gradient-to-r from-transparent via-transparent to-white lg:bg-gradient-to-r lg:from-transparent lg:via-white/20 lg:to-white" : "bg-gradient-to-t from-[#071a42]/85 via-transparent to-transparent"}`} /><div className={`absolute z-10 ${featured ? "bottom-0 left-0 max-w-[58%] p-5 text-[#071a42]" : "inset-x-0 bottom-0 p-4 text-white"}`}><h3 className="text-sm font-bold sm:text-base">{tool.title}</h3><p className={`mt-1 text-xs leading-5 ${featured ? "text-[#607493]" : "text-white/80"}`}>{tool.description}</p><span className={`mt-3 inline-flex items-center gap-1 text-xs font-semibold ${featured ? "text-[#148ee0]" : "text-white/90"}`}>Découvrir <ChevronRight className="h-3.5 w-3.5 transition group-hover:translate-x-0.5" /></span></div></Link>})}
          </div>
