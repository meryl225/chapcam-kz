...
export function ReferenceHomepage() {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#f4f9ff] text-[#071a42]">
      ...
      <main className="relative mx-auto max-w-[760px] px-4 pb-28 sm:px-6 lg:max-w-7xl lg:max-w-[1440px] lg:pb-12">
        <header className="flex h-[76px] items-center justify-between gap-4 border-b border-[#d8e8f6]/80">
          ...
          <nav className="hidden items-center gap-7 text-[13px] font-semibold text-[#536783] lg:flex" aria-label="Navigation principale">
            <Link href="#outils" className="transition-colors hover:text-[#10234d]">{t("Fonctionnalités")}</Link>
            <Link href="#founder" className="transition-colors hover:text-[#10234d]">{t("Founder")}</Link>
            <Link href="#a-propos" className="transition-colors hover:text-[#10234d]">{t("À propos")}</Link>
            <Link href="#faq" className="transition-colors hover:text-[#10234d]">{t("FAQ")}</Link>
          </nav>
          <div className="flex min-w-0 flex-nowrap items-center gap-2 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:min-w-fit lg:overflow-x-visible">
            ...
            <button type="button" onClick={() => setPlansOpen(true)} style={{ backgroundColor: "#075985", color: "#ffffff" }} className="rounded-xl border border-[#075985] px-3 py-2 text-[12px] font-bold shadow-[0_6px_18px_-10px_rgba(7,89,133,.9)] transition hover:-translate-y-0.5 hover:bg-[#0369a1] sm:px-3 sm:text-[13px]">{t("Tarifs")}</button>
            <Link href="/auth/login" className="shrink-0 whitespace-nowrap rounded-xl px-3 py-2 text-[13px] font-semibold text-[#53637c] transition hover:bg-white/70 hover:text-[#10234d]">{t("Se connecter")}</Link>
            <Link href="/auth/sign-up" className="whitespace-nowrap rounded-xl bg-gradient-to-r from-[#10a8ec] to-[#7c3aed] px-4 py-2.5 text-[13px] font-bold text-white shadow-[0_8px_24px_-8px_rgba(59,130,246,.7)] transition hover:-translate-y-0.5 hover:shadow-[0_12px_28px_-8px_rgba(59,130,246,.75)]">{t("S’inscrire")}</Link>
          </div>
        </header>

        <CreatorVideoStrip />

        <div className="mb-5 flex h-14 items-center gap-3 rounded-2xl border border-white/80 bg-white/65 px-4 shadow-[0_12px_30px_-20px_rgba(28,77,130,.55)] backdrop-blur"><Search className="h-5 w-5 text-[#6480a4]" /><span className="text-sm text-[#7487a1]">{t("Rechercher un outil, un effet, une idée...")}</span></div>
        ...
        <section className="relative overflow-hidden rounded-[1.7rem] border border-white/90 bg-[#dceeff] shadow-[0_24px_60px_-28px_rgba(37,99,235,.55)] lg:min-h-[500px]">
          ...
          <div className="relative z-20 max-w-xl px-5 pb-7 pt-8 sm:px-10 sm:pb-10 sm:pt-12 lg:max-w-[42%] lg:px-10 lg:py-24 xl:px-12"><span className="inline-flex items-center gap-2 rounded-full border border-[#4ec7f1]/40 bg-white/55 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wide text-[#087bad]"><Sparkles className="h-3.5 w-3.5" />{t("L'IA au service de ta créativité")}</span><h1 className="mt-5 text-[2.75rem] font-black leading-[.96] tracking-[-.06em] text-[#071a42] sm:text-6xl lg:text-[4.25rem]">{t("Crée")} <span className="bg-gradient-to-r from-[#079ded] to-[#7654e8] bg-clip-text text-transparent">{t("sans limites.")}</span></h1><p className="mt-5 max-w-xs text-sm leading-6 text-[#405675]">{t("Première plateforme d’IA dédiée aux transformations en temps réel et à la génération de vidéos full body Swap.")}</p><Link href="/auth/sign-up" className="mt-6 inline-flex items-center gap-3 rounded-xl bg-gradient-to-r from-[#0ea5e9] to-[#7c3aed] px-6 py-3.5 text-sm font-bold text-white shadow-[0_14px_28px_-12px_rgba(37,99,235,.75)]">{t("Commencer gratuitement")} <ChevronRight className="h-4 w-4" /></Link><div className="mt-6 flex flex-wrap gap-x-4 gap-y-2 text-[11px] font-medium text-[#526986]"><span>{t("Accès immédiat")}</span><span>{t("Outils IA créatifs")}</span></div></div><div className="relative h-[17rem] border-t border-white/60 bg-[#07111f] sm:h-[25rem] lg:absolute lg:inset-y-0 lg:right-0 lg:h-full lg:w-[58%] lg:border-t-0 lg:border-l lg:border-white/50"><div className="pointer-events-none absolute inset-y-0 left-0 z-10 hidden w-1/4 bg-gradient-to-r from-[#dceeff]/70 to-transparent lg:block" /><video src="/videos/img-1467.mp4" autoPlay muted loop playsInline preload="metadata" className="h-full w-full object-contain object-center" aria-label={t("Démonstration ChapCam")} /></div></section>

        <section id="outils" className="pt-16 lg:pt-24">
          <div className="mb-7 flex items-end justify-between gap-6">
            <div><h2 className="text-3xl font-bold tracking-[-.04em] text-[#071a42] sm:text-4xl">{t("Explore ChapCam")}</h2><p className="mt-2 text-sm text-[#607493]">{t("Tous tes outils créatifs, dans un seul espace.")}</p></div>
            <Link href="/dashboard" className="hidden items-center gap-1 text-sm font-semibold text-[#148ee0] sm:flex">{t("Voir tous les outils")} <ChevronRight className="h-4 w-4" /></Link>
          </div>
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-12 lg:grid-rows-[170px_170px_150px]">
            {tools.map((tool, index) => { const featured = index === 0; return <Link key={tool.title} href={tool.href} className={`group relative overflow-hidden rounded-[18px] border border-[#dbe9f4] bg-white shadow-[0_14px_32px_-24px_rgba(28,77,130,.6)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_38px_-22px_rgba(28,77,130,.55)] ${featured ? "col-span-2 min-h-[430px] bg-[#dceeff] lg:col-span-5 lg:row-span-2 lg:min-h-0" : "min-h-[170px] lg:col-span-4"}`}><div className="absolute inset-0">{tool.media.type === "video" ? <video src={tool.media.src} poster={"poster" in tool.media ? tool.media.poster : undefined} autoPlay muted loop playsInline preload="metadata" aria-label={tool.title} className={`h-full w-full transition duration-500 ${featured ? "bg-[#dceeff] object-contain object-center" : "object-cover group-hover:scale-105"}`} /> : <Image src={tool.media.src} alt={tool.title} fill className="object-cover transition duration-500 group-hover:scale-105" />}</div><div className={`absolute inset-0 ${featured ? "bg-gradient-to-t from-[#071a42]/75 via-transparent to-transparent lg:bg-gradient-to-r lg:from-transparent lg:via-white/20 lg:to-white" : "bg-gradient-to-t from-[#071a42]/85 via-transparent to-transparent"}`} />{featured && <div className="absolute inset-y-0 right-3 z-10 flex w-12 flex-col items-center justify-center gap-2 lg:right-5 lg:w-[36%] lg:flex-row lg:flex-wrap lg:content-center"><span className="mb-1 hidden text-center text-[9px] font-bold uppercase tracking-[.12em] text-[#607493] lg:block">{t("Compatible streaming")}</span>{streamingPlatforms.map(([name, src]) => <span key={name} className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/80 bg-white/85 p-2 shadow-[0_8px_18px_-10px_rgba(7,26,66,.55)]" title={name}><img src={src} alt={name} className="h-full w-full object-contain" /></span>)}</div>}<div className={`absolute z-10 ${featured ? "bottom-0 left-0 max-w-[58%] p-5 text-[#071a42]" : "inset-x-0 bottom-0 p-4 text-white"}`}><h3 className="text-sm font-bold sm:text-base">{t(tool.title)}</h3><p className={`mt-1 text-xs leading-5 ${featured ? "text-[#607493]" : "text-white/80"}`}>{t(tool.description)}</p><span className={`mt-3 inline-flex items-center gap-1 text-xs font-semibold ${featured ? "text-[#148ee0]" : "text-white/90"}`}>{t("Découvrir")} <ChevronRight className="h-3.5 w-3.5 transition group-hover:translate-x-0.5" /></span></div></Link>})}
          </div>
          <Link href="/dashboard" className="mt-5 flex items-center justify-center gap-1 text-sm font-semibold text-[#148ee0] sm:hidden">{t("Voir tous les outils")} <ChevronRight className="h-4 w-4" /></Link>
        </section>
      ...
      <HomepageFooter />
      <nav className="fixed inset-x-0 bottom-0 z-50 mx-auto flex max-w-[760px] items-center justify-around rounded-t-[1.7rem] border border-white/80 bg-white/85 px-3 py-3 shadow-[0_-12px_35px_-18px_rgba(28,77,130,.55)] backdrop-blur-xl lg:hidden"><Link href="/" className="flex flex-col items-center gap-1 text-xs font-semibold text-[#148ee0]"><Home className="h-5 w-5" />{t("Accueil")}</Link><Link href="/dashboard" className="flex flex-col items-center gap-1 text-xs text-[#71839e]"><Search className="h-5 w-5" />{t("Explorer")}</Link><Link href="/auth/sign-up" className="-mt-7 flex h-14 w-14 flex-col items-center justify-center rounded-full bg-gradient-to-br from-[#0ea5e9] to-[#7c3aed] text-white shadow-[0_10px_24px_-6px_rgba(37,99,235,.75)]"><Plus className="h-7 w-7" /><span className="sr-only">{t("Créer")}</span></Link><Link href="/dashboard" className="flex flex-col items-center gap-1 text-xs text-[#71839e]"><ImageIcon className="h-5 w-5" />{t("Mes créations")}</Link><Link href="/dashboard/settings" className="flex flex-col items-center gap-1 text-xs text-[#71839e]"><UserRound className="h-5 w-5" />{t("Profil")}</Link></nav>
