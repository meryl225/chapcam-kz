...
import { Check, Loader2, Film, ArrowRight } from 'lucide-react'
import { MOTION_OFFERS } from '@/lib/motion-offers'
import { usePaymentCheckout } from '@/components/payment/use-payment-checkout'

// Section d'achat de credits Motion Control, achetables SANS forfait Live Swap.
// Reutilise le tunnel de paiement partage (PayDunya / crypto).
export function MotionCreditPacksSection() {
  ...
  return (
    <section className="mt-6 rounded-2xl border border-hairline bg-card p-5 lg:p-6">
      ...
      <div className="mb-1 flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-foreground">Acheter des clips Motion</h2>
      </div>
      <p className="mb-5 text-sm text-muted-foreground text-pretty">
        Pas de forfait ? Achète directement des crédits Motion Control. 1 crédit = 1 clip de 10
        secondes (Standard ou Pro). Paiement sécurisé par mobile money, carte ou crypto.
      </p>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {MOTION_OFFERS.map((offer) => {
          const isPending = pendingKey === offer.id
          const pricePerClip = Math.round(offer.price / offer.credits)
          return (
            <div
              key={offer.id}
            ...
            >
              ...
              <div className="mt-2 flex items-baseline gap-1">
                ...
                <span className="text-sm text-muted-foreground">
                  {offer.credits > 1 ? 'clips de 10s' : 'clip de 10s'}
                </span>
              </div>
              ...
              <p className="text-xs text-text-faint">
                soit {pricePerClip.toLocaleString('fr-FR')} FCFA / clip
              </p>
