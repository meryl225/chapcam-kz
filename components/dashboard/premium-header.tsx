    ...
    <div className="mb-7 space-y-4">
      ...
      <section aria-label="Solde ChapCam" className="grid gap-3 lg:grid-cols-[1fr_1.15fr_0.82fr]">
        <div className="relative overflow-hidden rounded-xl border border-emerald-300/25 bg-[radial-gradient(circle_at_15%_0%,rgba(16,185,129,0.18),transparent_45%),linear-gradient(135deg,rgba(236,253,245,0.98),rgba(240,249,255,0.98))] dark:bg-[radial-gradient(circle_at_15%_0%,rgba(16,185,129,0.18),transparent_45%),linear-gradient(135deg,rgba(8,35,45,0.95),rgba(10,20,35,0.96))] p-4 shadow-[0_18px_45px_-24px_rgba(16,185,129,0.65),inset_0_1px_0_rgba(255,255,255,0.12)] before:pointer-events-none before:absolute before:inset-0 before:rounded-xl before:ring-1 before:ring-inset before:ring-emerald-200/10">
          <div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-full bg-[#0a1024] ring-1 ring-emerald-200/50"><img src="/images/jetons-logo.png" alt="Logo des Jetons" className="h-full w-full scale-[1.3] object-cover object-center" /></span><div><p className="text-sm font-semibold text-foreground"><T>Mes jetons</T></p><p className="mt-1 text-4xl font-black tabular-nums text-emerald-950 drop-shadow-[0_0_16px_rgba(110,231,183,0.45)] dark:text-emerald-100">{jetons?.balance ?? 0}</p></div></div><Link href="/dashboard/jetons" aria-label="Ajouter des jetons" className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-300 text-emerald-950 transition hover:scale-105"><Plus className="h-5 w-5" /></Link></div>
          <p className="mt-4 text-xs leading-5 text-emerald-900/80 dark:text-emerald-100/65"><T>Utilisables sur les outils ChapCam hors Live Swap</T></p>
        </div>
