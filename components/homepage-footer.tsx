    ...
    <footer className="relative mt-16 overflow-hidden border-t border-[#1b2945] bg-[#070c1b] px-6 py-12 text-[#e6edf9] sm:px-8 lg:px-12">
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-10 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex max-w-3xl flex-col items-start gap-7 lg:items-end">
          ...
          <a href="https://t.me/chapcam" target="_blank" rel="noreferrer" aria-label="Contacter ChapCam sur Telegram" className="group fixed bottom-6 right-6 z-50 flex min-h-16 items-center gap-3 rounded-full bg-[#070c1b]/90 py-1 pl-4 pr-1 text-[#c0c9d9] shadow-[0_10px_30px_rgba(0,0,0,.24)] backdrop-blur-md transition-transform hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#168dcc] focus-visible:ring-offset-2 focus-visible:ring-offset-[#070c1b] sm:bottom-8 sm:right-8">
            <span className="whitespace-nowrap text-sm italic transition-colors group-hover:text-white">Besoin d&apos;aide ?</span>
            <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full border-4 border-[#0b6fae] bg-[#168dcc] shadow-[0_0_28px_rgba(0,184,255,.55)] transition-transform group-hover:scale-105"><Send className="h-6 w-6 -rotate-12 fill-current text-white" aria-hidden="true" /></span>
          </a>
        </div>
      </div>
    </footer>
