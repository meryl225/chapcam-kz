...
export default function AdminPage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#070b14] p-4">
        <div className={`${card} w-full max-w-sm p-8 text-center`}>
          ...
          <p className="mt-1 text-sm text-white/50">Restricted area. Enter your admin passcode to continue.</p>
          <input
            type="password"
            value={code}
            autoFocus
          ...
          />
          {error && <p className="mt-2 text-xs text-red-400">Incorrect passcode. Hint: chapcam-admin</p>}
          <button
            onClick={unlock}
  ...
  const kpis = [
    { label: 'Total users', value: '12,480', icon: Users, sub: '+184 this week' },
    { label: 'MRR', value: formatUSD(totalRevenue), icon: DollarSign, sub: '+8.2%' },
    { label: 'Active numbers', value: '34,902', icon: Phone, sub: 'across 150 countries' },
    { label: 'Open risk events', value: ADMIN_RISK_EVENTS.length.toString(), icon: AlertTriangle, sub: '2 high severity' },
  ]
        ...
        <section className={`${card} overflow-hidden`}>
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/5 p-5">
            <h2 className="flex items-center gap-2 font-semibold">
              <Users className="h-4 w-4 text-blue-400" /> User management
            </h2>
              ...
              <tbody className="divide-y divide-white/5">
                {filteredUsers.map((u) => (
                  <tr key={u.id} className="text-white/70">
                    ...
                    <td className="p-4">
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${riskColor[u.risk]}`}>
                        {u.risk}
                      </span>
                    </td>
                    <td className="p-4">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <section className={`${card} overflow-hidden`}>
            <div className="border-b border-white/5 p-5">
              <h2 className="flex items-center gap-2 font-semibold">
                <AlertTriangle className="h-4 w-4 text-amber-400" /> Risk & fraud monitoring
              </h2>
            </div>
              ...
              {ADMIN_RISK_EVENTS.map((e) => (
                <li key={e.id} className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs font-medium capitalize ${sevColor[e.severity]}`}>{e.severity}</span>
                  </div>
          ...
          <div className="space-y-4">
            <section className={`${card} p-5`}>
              <h2 className="mb-3 flex items-center gap-2 font-semibold">
                <Activity className="h-4 w-4 text-blue-400" /> Provider health
              </h2>
