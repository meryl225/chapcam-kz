...
/** Lit la durée (en secondes) d'un fichier vidéo côté client via un <video> caché. */
...
export default function GenjutsuPage() {
  ...
  const generate = async () => {
    ...
    // Genjutsu est un transfert de mouvement : la vidéo de référence est
    // indispensable, c'est elle qui fournit le mouvement à appliquer à l'image.
    if (!reference) {
      setMessage('Ajoute une vidéo de mouvement : Genjutsu transfère son mouvement sur ton image.')
      return
    }
    // Higgsfield refuse (échec silencieux après lancement) les vidéos hors de la
    // plage 4 à 30 secondes. On vérifie la durée avant d'envoyer et de débiter.
    const duration = await readVideoDuration(reference).catch(() => null)
    if (duration !== null && (duration < 4 || duration > 30)) {
      setMessage(`Ta vidéo dure ${Math.round(duration)} s. Genjutsu exige une vidéo de 4 à 30 secondes.`)
      return
    }
    setLoading(true)
          ...
          <section className="rounded-3xl border border-white/10 bg-white/[0.035] p-4 shadow-2xl shadow-black/20 md:p-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <button type="button" onClick={() => document.getElementById('reference-video')?.click()} className="group flex min-h-72 flex-col items-center justify-center overflow-hidden rounded-2xl border border-dashed border-white/25 bg-white/[0.03] p-5 text-center transition duration-200 hover:-translate-y-0.5 hover:border-[#c6f542]/80 hover:bg-[#c6f542]/[0.08] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c6f542]">
                {referencePreview ? <video src={referencePreview} controls className="h-full max-h-72 w-full object-cover" /> : <><Video className="mb-3 h-10 w-10 text-[#c6f542]" /><strong>Vidéo de mouvement <span className="text-[#c6f542]">*</span></strong><span className="mt-2 text-center text-xs text-white/40">Requise · de 4 à 30 secondes</span><span className="mt-4 inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/10 px-4 py-2.5 text-xs font-bold text-white shadow-sm transition group-hover:border-[#c6f542]/40 group-hover:bg-[#c6f542]/15"><Upload className="h-3.5 w-3.5" /> Importer une vidéo</span></>}
              </button>
            </div>
